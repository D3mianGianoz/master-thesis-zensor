...
As the section title states, here we propose a possible approach to monitor the energy consumption of a large campus (\acs{BHC} \todo{EB: quale citt\`a, stato?}) using existing informations, that have been collected over several years.
...
Furthermore, this \todo{EB: ``this'' what? Aggiungere soggetto. ``hospital''? ``cabin''?} can also have connections with one or more ``sub'' transformers, like the one here on the left, 
...
Data is provided pre-packaged, saving a considerable amount of time needed for data-collection, \todo{EB: non capisco Inglese next ``but there with less control over its consistency and quality'' mi pare manchi un verbo} but there with less control over its consistency and quality.
...
With these considerations,  we can consider this paragraph complete \todo{EB: meglio ``we can close the discussion'' o simili?}, knowing that a few aspects have been left out, in order to keep the text as relevant and clean as possible.
...
As evidence of this, they also decided to further expand the plan by adding a considerable amount of \ac{GEP} data, which became an sextant \todo{EB: sicuro del ``sextant''?} sub-project.
