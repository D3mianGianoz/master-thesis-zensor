...
The subject of this monitoring operation is a blade-making machine line.
...
\todo{EB: continuo a non capire ``ad-hoc''}
...
\begin{itemize}
    ...
    \item[$\circledcirc$] Lastly, as preliminary path for the \ac{CtM}, identify purpose. \todo{EB: questo punto non l'ho capito}
\end{itemize}
...
\end{figure}
As we saw earlier in section~\ref{section:zensor_approach}, we can have two types of projects and this one a ``complete'' one. \todo{EB: ricordare cosa vuol dire un ``complete one''}
\todo{EB: troppo informale ``as the reader will have already guessed''. E fra l'altro, io reader non solo non ho ``guessed'' , ma ci ho anche capito poco. Perch\'e less complex than big project? E cosa \`e un ``big'' project? Magari era scrtto prima. Ma bene sia ricordarlo che aggiungere riferimento a ``prima''}
...
a somewhat complex issue that has caused several \todo{EB: ``headaches'' informal} headaches in the analysis phase. This would therefore appear to be a good point to take stock of the situation.
The \todo{EB: ``accelerometer''?} accelero was installed within the line, adjacent to the stones and close to the blade, by means of a round magnet, as shown in Figure \ref{fig:s1b_foto}.
...
\end{itemize}
The most attentive reader will have already grasped the different references present in the mentioned figures alongside. % here on the side 
This dual model well abstracts the reality, but does not allow a comparison between the various stations, \todo{EB: ``ergo''? Meglio parole normali} ergo to achieve one of the initial goals.
...
\begin{itemize}
    ...
    the help of a level. \todo{EB: check English: ``tolerates''?} This tolerating some errors due to disturbing factors such as color, dirt and imperfections of the metal.
...
Moreover, for each sensor channel, all vibrations are recorded at \texttt{60hz}, \todo{EB: gli Hertz normalmente si abbreviano con ``Hz'' non ``hz''} or 60 points per second, for a total of 180 per second (\ref{sub@fig:stu_60hz_raw}).
...
Let's go through the procedure together. \todo{EB: troppo informale}
...
\end{enumerate}
Let's take a closer look at the various operations: % troppo informale? EB: si
...
After all of these operations, we combined \texttt{1hz} numerical values to the previously prepared metadata to achieve an interesting midterm result that has approximately this aspect. \todo{EB: ``midterm'' ==> ``intermediate''?}
