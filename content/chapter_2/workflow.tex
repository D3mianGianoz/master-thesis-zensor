\section{Workflow}
As we will see better in Chapter~\ref{chapter:use_cases} dedicated to practical use cases, where will give more space to some technical details, my internship has focused more on the last two phases, 
out of the four discussed previously \todo{EB2DG: ``last two phases'' di cosa? Conviene ricordarlo, a meno che non sia banale. Quali sono le fasi?}, for reasons of limited time and domain knowledge. Now, in this last section, 
I would like to present some peculiar elements and techniques that zensor employs for the 3rd and 4th phases: data management and analysis.
...
The contains two functions that are designed to ease writing scripts that generate reproducible time ranges, ensuring that analysis is always aligned on the correct time window:
\todo{EB2DG: itemize? enumerate?}
the first one will generate evenly spaced timestamps in a given range, while the second one, instead generated pairs of times, which are separated by the argument (time) \texttt{interval}, the ``amplitude'' of the window.
The latter can be useful e.g.\ to generate a window for the current year/month/week to date.
...
% The goal of a good script should be that this happens in a reliably way regardless of when exactly the script is run. 
